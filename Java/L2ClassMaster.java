...
public final class L2ClassMasterInstance extends L2NpcInstance
{
			...
			switch (jobLevel)
			{
				case 0:
					if (Config.CLASS_MASTER_SETTINGS.isAllowed(1))
						sb.append("Apenas jogadores do Level 20 ou superior podem realizar a primeira troca de classe.<br>");
					else if (Config.CLASS_MASTER_SETTINGS.isAllowed(2))
						sb.append("realize sua primeira troca de classe antes.<br>");
					else if (Config.CLASS_MASTER_SETTINGS.isAllowed(3))
						sb.append("realize sua segunda troca de classe antes.<br>");
					else
						sb.append("indisponivel.<br>");
					break;
				case 1:
					if (Config.CLASS_MASTER_SETTINGS.isAllowed(2))
						sb.append("Apenas jogadores do Level 45 ou superior podem realizar a segunda troca de classe.<br>");
					else if (Config.CLASS_MASTER_SETTINGS.isAllowed(3))
						sb.append("realize sua primeira troca de classe antes.<br>");
					else
						sb.append("indisponivel.<br>");
				...
				case 2:
					if (Config.CLASS_MASTER_SETTINGS.isAllowed(3))
						sb.append("Apenas jogadores do Level 76 ou superior podem realizar a segunda troca de classe.<br>");
					else
						sb.append("indisponivel.<br>");
					break;
				case 3:
					sb.append("Todas as Trocas de Classes foram Realizadas.<br>");
					break;
			}
			sb.append("</body></html>");
					...
					for (ClassId cid : ClassId.values())
					{
						if (validateClassId(currentClassId, cid) && cid.level() == level)
						{
							StringUtil.append(menu,
									"<button value=\" ",CharTemplateTable.getInstance().getClassNameById(cid.getId()),
									"\" action=\"bypass -h npc_%objectId%_change_class ",String.valueOf(cid.getId()),
									"\"  width=150 height=40 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"><br>"
									);
						}
					}
	...
	private static String getRequiredItems(int level)
	{
		if (Config.CLASS_MASTER_SETTINGS.getRequireItems(level) == null || Config.CLASS_MASTER_SETTINGS.getRequireItems(level).isEmpty())
			return "<tr><td>Sem Requisitos</td></r>";
		...
		for (int _itemId : Config.CLASS_MASTER_SETTINGS.getRequireItems(level).keys())
		{
			...
			sb.append(
					"<tr>  <td><font color=\"00BFFF\">" + _count + "</font> </td><td></td> <td><font color=\"LEVEL\">" + ItemTable.getInstance().getTemplate(_itemId).getName() + "</font></td></tr>"
					);
		}
		return sb.toString();
	}
