/*
  ...
  * os termos da GNU General Public License conforme publicado pela Free Software
  * Foundation, seja a versão 3 da Licença, ou (a seu critério) qualquer versão posterior
  * versão.
  *
  * Este programa é distribuído na esperança de que seja útil, mas SEM
  * QUALQUER GARANTIA; sem mesmo a garantia implícita de COMERCIALIZAÇÃO ou ADEQUAÇÃO
  * PARA UM PROPÓSITO ESPECÍFICO. Veja a Licença Pública Geral GNU para mais
  * detalhes.
  *
...
public final class QuestState
{
	...
	/** Quest associada a QuestState */
	private final String _questName;

	/** Objeto no caso, o player que esta na quest */
	private final L2PcInstance _player;

	/** Estado da Quest */
	private byte _state;

	/** couples (variável para busca, valor da variável para busca) */
	private Map<String, String> _vars;

    /** Sinalizador booleano informando ao QuestStateManager para limpar tudo ao sair da missão */
    private boolean _isExitQuestOnCleanUp = false;

    /**
	 * Constructor of the QuestState : salve a missão na lista de missões do jogador.<BR/><BR/>
	 *
	 * <U><I>Ações :</U></I><BR/>
	 * <LI>Salve as informações no objeto QuestState criado (Quest, Player, Completion, State)</LI>
	 * <LI>Adicione o QuestState na lista de missões do jogador usando setQuestState()</LI>
	 * <LI>Adicione drops obtidos pela missão</LI>
	 * <BR/>
	 * @param quest : missão associada ao QuestState
	 * @param player : L2PcInstance apontando o player
	 * @param state : Estado da quest
	 * @param completed : boolean para completar a quest
	 */
    QuestState(Quest quest, L2PcInstance player, byte state)
    {
		...
		// Salve o estado da missão para o jogador na lista de missões iniciadas do jogador
		// o this é usado para instanciar o proprio metodo e atributos da propria instancia
        getPlayer().setQuestState(this);

		// seta o estado da quest
		_state = state;
    }
	...
	/**
	 * Retorna true se a missão for concluída, false caso contrário
	 * @return boolean
	 */
	...
	/**
	 * Retorna true se a missão começou, false caso contrário
	 * @return boolean
	 */
	...
	/**
	 * Retorna o estado da quest após sua inicialização.<BR><BR>
	 * <U><I>Ações :</I></U>
	 * <LI>Remover drops do estado anterior</LI>
	 * <LI>Definir novo estado da missão</LI>
	 * <LI>Adicionar drop para novo estado</LI>
	 * <LI>Atualizar informações no banco de dados</LI>
	 * <LI>Enviar pacote QuestList para client</LI>
	 * @param state
	 * @return object
	 */
	public Object setState(byte state)
    {
        // definir novo estado se ainda não estiver nesse estado
		if (_state != state)
		{
		    _state = state;
...
// END STUFF THAT WILL PROBABLY BE CHANGED
    ...
    /**
      * Adicione spawn para a instância do jogador
      * Desaparecerá após a duração do spawn expirar
      * Usa as coordenadas e o heading do jogador.
      * Adiciona um pouco de randomização nas coordenadas x y
      * Retorna o id do objeto do npc recém-gerado
      */
...
/**
      * Adicione spawn para a instância do jogador
      * Herda as coordenadas e o heading da instância L2Character especificada.
      * Pode ser o jogador ou qualquer mob morto/atacado
      * Retorna o id do objeto do npc recém-gerado
      */
